...
def is_path_safe(base, path):
	...
	if base == '/':
		for allowed in allowed_paths:
			if final_path.startswith(allowed):
				return True
		return False

	base = os.path.normpath(base)
	return final_path == base or final_path.startswith(base + os.sep)

def safe_tar_extract(src, dest):
	if not os.path.exists(src):
		return
	try:
		with tarfile.open(src, 'r:gz') as tar:
			for member in tar.getmembers():
				if is_path_safe(dest, member.name):
					tar.extract(member, dest)
	except Exception as e:
		print("[safe_tar_extract] Error: %s" % str(e))

def safe_zip_extract(src, dest):
	if not os.path.exists(src):
		return
	try:
		with zipfile.ZipFile(src, 'r') as zip_ref:
			for member in zip_ref.namelist():
				if is_path_safe(dest, member):
...
def remove_wildcard(pattern):
	for f in glob.glob(pattern):
		try:
			...
			print("[remove_wildcard] Error: %s" % str(e))
			...
			with open('/proc/mounts', 'r') as f:
				for line in f.readlines():
					if line.find('/media/cf') != -1:
						...
						myhdd2 = '/media/hdd2/'
					elif line.find('/media/hdd') != -1 or line.find('/hdd') != -1:
						myhdd = '/media/hdd/'
...
class ChannelListDownloadTask(Task):
	...
	def download_progress(self, recvbytes, totalbytes):
		if totalbytes > 0 and ( recvbytes - self.last_recvbytes  ) > 10000: # anti-flicker
