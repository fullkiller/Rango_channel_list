...
try:
    ...
    MessageBox = InfoBar = JobView = ChoiceBox = TryQuitMainloop = None

try:
    from Components.Label import Label
    from Components.ActionMap import ActionMap
except ImportError:
    Label = ActionMap = None

try:
    from Components.Task import Task, Job, job_manager, Condition
except ImportError:
    Task = Job = job_manager = Condition = None

try:
    from Tools.Downloader import downloadWithProgress
    from Tools.Directories import fileExists
except ImportError:
    downloadWithProgress = fileExists = None
...
def is_path_safe(destination, base_path):
    ...
    if base_path == '/':
        ...
        for prefix in allowed_prefixes:
            norm_prefix = os.path.abspath(prefix)
            if destination == norm_prefix or destination.startswith(norm_prefix + os.sep) or destination.startswith(norm_prefix + '/'):
                return True
        return False
    if destination == base_path:
        return True
    if destination.startswith(base_path + os.sep) or destination.startswith(base_path + '/'):
        return True
    return False

def safe_tar_extract(archive_path, base_path):
    t = tarfile.open(name=archive_path, mode="r:gz")
    members = []
    for member in t.getmembers():
        target_path = os.path.join(base_path, member.name)
        if is_path_safe(target_path, base_path):
            members.append(member)
        else:
            print("[ChannelListUpdate] skipping unsafe member: %s" % member.name)
    t.extractall(path=base_path, members=members)
    t.close()

def safe_zip_extract(archive_path, base_path):
    zf = zipfile.ZipFile(archive_path)
    members = []
    for member_name in zf.namelist():
        target_path = os.path.join(base_path, member_name)
        if is_path_safe(target_path, base_path):
            members.append(member_name)
        else:
            print("[ChannelListUpdate] skipping unsafe member: %s" % member_name)
    zf.extractall(path=base_path, members=members)
    zf.close()
...
def remove_wildcard(pattern):
    for path in glob.glob(pattern):
        try:
            ...
            print("[ChannelListUpdate] Error removing %s: %s" % (path, str(e)))
...
class CommonChannelListScreen(Screen):
	...
	def red(self):
		...
		try:
			...
			print("[ChannelListUpdate] Error in red callback: ", e)
	...
	def green(self):
		...
		try:
			...
			print("[ChannelListUpdate] Error in green callback: ", e)
			self["status"].setText('Status: Nie mogę się połączyć z serwerem !')
			self.reloadUrlList(self.url1, self.url2)
	...
	def versionInfoCallBack(self, html, ltype="1x"):
		try:
			html_str = decode_html(html)
			new = dict([[x.strip() for x in l.split(':', 1)] for l in html_str.split("\n") if l.find(":") > 0])
			if 'version' in new:
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			...
			print("[ChannelListUpdate] downloadCallBack error: ", e)
			self["status"].setText('Status: Błąd archiwum !')

	def errorUpdate(self, html=None):
		self["status"].show()
		self["status"].setText('Status: Nie mogę się połączyć z serwerem !')
...
class Djcrash(Screen):
	...
	def downloadFromUpload(self, failure=None):
		...
		downloadPage(url.encode(), b'/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromPKT_once)
	...
	def downloadFromPKT(self, failure=None):
		...
		downloadPage(url.encode(), b'/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromUpload_once)

	def downloadFromPKT_once(self, failure=None):
		self["status"].setText("Status: Alternatywne pobieranie listy " + self.chooseList)
		url = "http://addon.pkteam.pl/channel_lists_e2/enigma2_list_by_djcrash-" + self.newVersion + ".zip"
		downloadPage(url.encode(), b'/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFailed)
	...
	def downloadFailed(self, failure=None):
		self["status"].setText("Status: Błąd pobierania listy!")

	def installChList(self, html):
		self["status"].setText('Status: Rozpakowywanie')
		try:
			safe_zip_extract(b"/tmp/lista.zip", "/tmp/chList/")
		except Exception as e:
			print("[ChannelListUpdate] Djcrash unzip error: ", e)
			self["status"].setText('Status: Błąd archiwum !')
			return
		...
		try:
			...
			print("[ChannelListUpdate] install error: ", e)
...
class CommonPiconListScreen(Screen):
				...
				for line in f:
					...
					if len(parts) >= 2:
						...
						if mp == '/media/cf':
							...
							myusb = '/media/usb/'
						elif mp in ('/media/card', '/media/sd'):
							mysd = '/media/card/'
						elif mp == '/media/hdd2':
							myhdd2 = '/media/hdd2/'
						elif mp in ('/media/hdd', '/hdd'):
							myhdd = '/media/hdd/'
	...
	def prepareDownload(self):
		...
		try:
			...
			print("[ChannelListUpdate] prepareDownload error: ", e)
			self["status"].setText('Status: Nie mogę się połączyć z serwerem !')
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			target_dir = os.path.join(self.piconLocationToInstall, "picon")
