...
class Djcrash(Screen):
	...
	def downloadFromUpload(self):
		...
		downloadPage(url.encode('utf-8'), '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.errorUpdate)
			...
			with open('/proc/mounts', 'r') as f:
				for line in f.readlines():
					if line.find('/media/cf') != -1:
						...
						myusb2 = '/media/usb2/'
					elif line.find('/media/usb3') != -1:
						myusb3 = '/media/usb3/'
					elif line.find('/media/card') != -1:
						mysd = '/media/card/'
					elif line.find('/hdd2') != -1:
						myhdd2 = '/media/hdd2/'
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			if self.piconTypeToInstall == "ZZPiconGLASS":
				dest = dest.replace("ZZPiconGLASS", "ZZPicon")
			elif self.piconTypeToInstall == "ZZPiconBLACK":
				dest = dest.replace("ZZPiconBLACK", "ZZPicon")
			elif self.piconTypeToInstall == "ZZPiconRJ":
				dest = dest.replace("ZZPiconRJ", "ZZPicon")
			elif self.piconTypeToInstall == "ZZPiconChristmas":
				dest = dest.replace("ZZPiconChristmas", "ZZPicon")
			elif self.piconTypeToInstall == "XPiconsBLACK":
				dest = dest.replace("XPiconsBLACK", "XPicons")
			elif self.piconTypeToInstall == "SetoXPicons":
				dest = dest.replace("SetoXPicons", "XPicons")

			picon_path = os.path.join(dest, "picon")
