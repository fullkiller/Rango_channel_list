...
def is_path_safe(base_path, path):
	...
	try:
		with tarfile.open(tar_file, 'r:gz') as tar:
			for member in tar.getmembers():
				...
				if is_path_safe(path, member_path):
					tar.extract(member, path)
	except Exception as e:
		print("Error extracting tar %s: %s" % (tar_file, e))
...
def safe_zip_extract(zip_file, path):
	try:
		...
		print("Error extracting zip %s: %s" % (zip_file, e))
