...
import traceback
import re, os, tarfile, zipfile, shutil, hashlib
...
def is_path_safe(base, path):
	...
	if not base.endswith(os.path.sep):
		base_with_sep = base + os.path.sep
	else:
		base_with_sep = base

	if base != os.path.abspath("/"):
		if path.startswith(base_with_sep) or path == base:
			return True
	...
	if base == os.path.abspath("/"):
		for allowed in allowed_paths:
			...
			if not allowed_abs.endswith(os.path.sep):
				allowed_abs_sep = allowed_abs + os.path.sep
			else:
				allowed_abs_sep = allowed_abs
			if path.startswith(allowed_abs_sep) or path == allowed_abs:
				return True
		return False
	return False
...
def safe_tar_extract(tar, path=".", members=None, numeric_owner=False):
	for member in tar.getmembers():
		...
		if not is_path_safe(path, member_path):
			print("[ChannelListUpdate] Security Warning: Path traversal attempt in tar archive: %s" % member.name)
			continue
...
def safe_zip_extract(zf, path="."):
	for member in zf.namelist():
		...
		if not is_path_safe(path, member_path):
			print("[ChannelListUpdate] Security Warning: Path traversal attempt in zip archive: %s" % member)
			continue
...
def verify_md5(filepath, expected_md5):
	...
	try:
		...
		print("[ChannelListUpdate] MD5 verification failed: %s" % str(e))
		return False
...
def remove_wildcard(pattern):
	...
	for f in glob.glob(pattern):
		try:
			...
			print("[ChannelListUpdate] Failed to remove %s: %s" % (f, str(e)))
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			if self.cleanup_before_install:
				# remove_wildcard('/etc/enigma2/*.tv') # Intentionally commented out
			...
			with open('/proc/mounts', 'r') as f:
				for line in f.readlines():
					if '/media/cf' in line:
						...
						myhdd2 = '/media/hdd2/'
					elif '/hdd' in line or '/media/hdd' in line:
						myhdd = '/media/hdd/'
