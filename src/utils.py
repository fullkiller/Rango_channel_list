...
def verify_md5(filepath, expected_md5):
	if not expected_md5 or expected_md5 == "00000000000000000000000000000000":
		return True
			...
			with open("/tmp/revision", 'r') as f:
				...
				for path in paths:
					if "=" in path:
						...
						if key == "DATE":
							self.newVersion = val.replace('"', "")
						elif key == "VER":
							self.chID = val.replace('"', "")
		...
		try:
			...
			for t in ["ZZPiconGLASS", "ZZPiconBLACK", "ZZPiconRJ", "ZZPiconChristmas"]:
				if self.piconTypeToInstall == t:
					dest = dest.replace(t, "ZZPicon")
			for t in ["XPiconsBLACK", "SetoXPicons"]:
				if self.piconTypeToInstall == t:
					dest = dest.replace(t, "XPicons")
