...
def safe_zip_extract(zip_file, path):
	with zipfile.ZipFile(zip_file) as zf:
		for member in zf.infolist():
			if is_path_safe(path, os.path.join(path, member.filename)):
...
class CommonChannelListScreen(Screen):
	...
	def red(self):
		...
		try:
			if self.url1x != '' and self.canDownload == 1:
				...
				downloadPage(self.url1x.encode(), b'/tmp/chList1x1.tgz').addCallback(self.downloadCallBack, "/tmp/chList1x1.tgz").addErrback(self.errorUpdate, self.url1, self.url2)
		except:
	...
	def green(self):
		...
		try:
			if self.url2x != '' and self.canDownload == 1:
				self["status"].setText('Status: Pobieranie listy')
				downloadPage(self.url2x.encode(), b'/tmp/chList2x1.tgz').addCallback(self.downloadCallBack, "/tmp/chList2x1.tgz").addErrback(self.errorUpdate, self.url1, self.url2)
		except:
			self["status"].setText('Status: Nie mogę się połączyć z serwerem !')
			self.reloadUrlList(self.url1, self.url2)
	...
	def reloadUrlList(self, url1, url2):
		...
		getPage(self.url1.encode()).addCallback(self.versionInfoCallBack, "1x").addErrback(self.errorUpdate, self.url1, self.url2)
		getPage(self.url2.encode()).addCallback(self.versionInfoCallBack, "2x").addErrback(self.errorUpdate, self.url1, self.url2)

	def versionInfoCallBack(self, html, ltype="1x"):
		try:
			html = decode_html(html)
			new = dict([ (l.split(':',1)[0].strip(), l.split(':',1)[1].strip()) for l in html.split("\n") if l.find(":") > 0])
			if 'version' in new:
	...
	def errorUpdate(self, html, url1=None, url2=None):
		self["status"].show()
		self["status"].setText('Status: Nie mogę się połączyć z serwerem !')
...
class Djcrash(Screen):
	...
	def installChList(self, html):
		...
		for f in glob.glob(src_dir + '*'):
			dest = os.path.join('/etc/enigma2/', os.path.basename(f))
			if os.path.isdir(f):
