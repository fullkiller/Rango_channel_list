...
try:
	from .utils import reloadChannelList, CommonChannelListScreen, Djcrash, CommonPiconListScreen, decode_html, verify_md5
	from . import _
except ImportError:
	from utils import reloadChannelList, CommonChannelListScreen, Djcrash, CommonPiconListScreen, decode_html, verify_md5
...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		...
		if not verify_md5(self.dlfile, self.MD5):
			self.downloadFailed(None, _("MD5 checksum mismatch!"))
			return
		...
		try:
			...
			self.downloadFailed(None, str(e))
			return
