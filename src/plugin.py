...
class ChannelListUpdateMenu(Screen):
	...
	def updateList(self):
		self.list = []
		mypath = resolveFilename(SCOPE_PLUGINS, "Extensions/RangoPolishChannelsUpdater/images/")
...
class Master(CommonChannelListScreen):
	def __init__(self, session, args = None):
		...
		self.reloadUrlList(self.url1, self.url2)
		self.cleanup_before_install = False
...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		...
		try:
			if self.MD5 and self.MD5 != "000000000000000000000000":
				import hashlib
				hash_md5 = hashlib.md5()
				with open('/tmp/update_rango.tar.gz', "rb") as f:
					for chunk in iter(lambda: f.read(4096), b""):
						hash_md5.update(chunk)
				downloaded_md5 = hash_md5.hexdigest()
				if downloaded_md5 != self.MD5:
					self['status'].setText(_('Błąd MD5 pobranego pliku!'))
					return

			import tarfile
			t = tarfile.open('/tmp/update_rango.tar.gz', 'r:gz')
			safe_tar_extract(t, '/')
			t.close()
		except Exception as error:
			print(error)
