...
try:
    ...
    getDesktop = RT_HALIGN_LEFT = RT_VALIGN_TOP = gFont = None

try:
    from Components.MultiContent import MultiContentEntryText, MultiContentEntryPixmapAlphaBlend
except ImportError:
    MultiContentEntryText = MultiContentEntryPixmapAlphaBlend = None
...
try:
    ...
    Label = ActionMap = List = Progress = StaticText = None
...
try:
    ...
    TryQuitMainloop = MessageBox = None
    class Screen(object):
...
try:
    ...
    LoadPixmap = fileExists = SCOPE_PLUGINS = resolveFilename = downloadWithProgress = None
...
try:
    from utils import reloadChannelList, CommonChannelListScreen, Djcrash, CommonPiconListScreen, decode_html, safe_tar_extract, remove_wildcard
except ImportError:
    from .utils import reloadChannelList, CommonChannelListScreen, Djcrash, CommonPiconListScreen, decode_html, safe_tar_extract, remove_wildcard
...
class ChannelListUpdateUpdater(Screen):
	def __init__(self, session, updateurl, md5_hash=""):
	...
	def downloadFinished(self, string = ''):
		...
		try:
			...
			if self.md5_hash and file_hash != self.md5_hash:
				self['status'].setText(_('Błąd sumy kontrolnej MD5 pobranego pliku!'))
				return
		except Exception as e:
			print("[ChannelListUpdate] MD5 check failed: ", e)

		try:
			safe_tar_extract(self.dlfile, "/")
		except Exception as e:
			print("[ChannelListUpdate] safe_tar_extract failed: ", e)
			self['status'].setText(_('Błąd rozpakowywania aktualizacji!'))
			return
	...
	def downloadProgress(self, recvbytes, totalbytes):
		if totalbytes > 0:
			...
			self['progresstext'].text = '%d kBytes' % (recvbytes / 1024)
