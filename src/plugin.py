...
from . import _
from .utils import reloadChannelList, CommonChannelListScreen, Djcrash, CommonPiconListScreen, safe_tar_extract, verify_md5, decode_html
import os
...
class ChannelListUpdateMenu(Screen):
	...
	def __init__(self, session):
		...
		self["info"] = Label(_("Plugin do aktualizacji list kanałow tworzonych przez HSWG, DjCrash'a, Krzyśka80 ,fullkiller™ i @Seto. Bez ich pracy nie było by list kanałów.\n\nRango - %s") % version)
	...
	def check_updates(self, tryb = 1):
		try:
			...
			if tryb == 0:
				getPage(self.url.encode()).addCallback(self.versionInfoCallBackQuiet).addErrback(self.errorUpdate)
			else:
				getPage(self.url.encode()).addCallback(self.versionInfoCallBack).addErrback(self.errorUpdate)
	...
	def processVersionInfo(self, data):
		...
		self.fileUrl = ""
		self.MD5 = ""
		content = decode_html(data)
		for line in content.splitlines():
			if ' = ' in line:
				key, value = line.split(' = ', 1)
				value = value.replace('"', '').strip()
				if key == "Version": self.newVersion = value
				elif key == "File": self.newFile = value
				elif key == "Url": self.fileUrl = value
				elif key == "MD5": self.MD5 = value
	...
	def versionInfoCallBackQuiet(self, html):
		...
		if int(self.newVersion) > int(version):
			...
			version,
			_('Aktualizacja jest dostępna!\n\nCzy chcesz uruchomić akutalizację teraz?'))
...
class ChannelListUpdateUpdater(Screen):
	...
	def startUpdate(self):
		...
		self.dlfile = b'/tmp/update_rango.tar.gz'
		self.download = downloadWithProgress(self.updateurl.encode(), self.dlfile)
	...
	def downloadFinished(self, string = ''):
		if not verify_md5(self.dlfile, self.MD5):
			self['status'].setText(_('Błąd sumy kontrolnej MD5!'))
			return
		...
		try:
			...
			self['status'].setText(_('Błąd instalacji: ') + str(e))
