...
class ChannelListUpdateMenu(Screen):
	...
	def parseVersionInfo(self, html):
		...
		if hasattr(config, 'read_file'):
			config.read_file(StringIO(html))
		else:
			config.readfp(StringIO(html))
	...
	def versionInfoCallBack(self, html):
		try:
			...
			if int(self.newVersion) == int(version):
				...
				_('Wersja zainstalowana:'),
				version,
				_('Posiadasz aktualna wersję pluginu !'))
...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		...
		try:
			t = tarfile.open(name = self.dlfile, mode = "r:gz")
			safe_tar_extract(t, "/")
			t.close()
			...
			self['status'].setText(_('Błąd podczas instalacji: %s' % str(e)))
