...
class ChannelListUpdateMenu(Screen):
	...
	def processVersionInfo(self, html):
		...
		if self.fileUrl.endswith("/") and self.newFile.startswith("/"):
			self.updateurl = self.fileUrl + self.newFile[1:]
		elif not self.fileUrl.endswith("/") and not self.newFile.startswith("/"):
			self.updateurl = self.fileUrl + "/" + self.newFile
...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		...
		safe_tar_extract('/tmp/update_rango.tar.gz', '/')
		os.system('sync')
