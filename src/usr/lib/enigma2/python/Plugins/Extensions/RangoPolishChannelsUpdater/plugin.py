...
class ChannelListUpdateMenu(Screen):
	...
	def parseVersion(self, html):
		...
		self.newVersion = config.get('plugin', 'Version').strip()
		self.newFile = config.get('plugin', 'File').strip()
		self.fileUrl = config.get('plugin', 'Url').strip()
		self.MD5 = config.get('plugin', 'MD5').strip()
...
class Seto(CommonChannelListScreen):
	def __init__(self, session, args = None):
		...
		self.url1 = 'https://raw.githubusercontent.com/fullkiller/Rango_channel_list/main/latest8x.txt'
