...
from .utils import reloadChannelList, CommonChannelListScreen, Djcrash, CommonPiconListScreen, safe_tar_extract, my_sync
import os
...
class ChannelListUpdateMenu(Screen):
	...
	def updateList(self):
		...
		for idx, name, desc, img in [
			...
			("masterpicon", "Pobierz Picony od @Seto i RJ", "Picony dla HotBird 13.0E oraz Astra 19.2E", "masterpicon.png"),
		]:
			mypixmap = mypath + img
			if not os.path.exists(mypixmap):
				if img == "masterpicon.png": mypixmap = mypath + "picon.png"
	...
	def parseVersionInfo(self, html):
		try:
			content = html.decode('utf-8')
			cp = configparser.ConfigParser()
			cp.read_file(io.StringIO(u'[plugin]\n' + content))
			if cp.has_section('plugin'):
				self.newVersion = cp.get('plugin', 'Version', fallback='0')
				self.newFile = cp.get('plugin', 'File', fallback='')
				self.fileUrl = cp.get('plugin', 'Url', fallback='').rstrip('/')
				self.MD5 = cp.get('plugin', 'MD5', fallback='')
		except:
			...
			for line in content.split('\n'):
				if '=' in line:
					...
					k = k.strip()
					v = v.strip().replace('"', '')
					if k == 'Version': self.newVersion = v
					elif k == 'File': self.newFile = v
					elif k == 'Url': self.fileUrl = v.rstrip('/')
					elif k == 'MD5': self.MD5 = v

	def versionInfoCallBackQuiet(self, html):
		self.parseVersionInfo(html)
...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		...
		try:
			t = tarfile.open(self.dlfile, "r:gz")
			safe_tar_extract(t, "/")
			t.close()
...
def Plugins(**kwargs):
	...
	return [
		...
		PluginDescriptor(name = "Polskie listy kanałów", description=_('Najnowsza kolekcja polskich list kanałów'), icon=icon, where=PluginDescriptor.WHERE_PLUGINMENU, fnc=main)
	]
