...
class ChannelListUpdateMenu(Screen):
	...
	def check_updates(self, tryb=1):
		try:
			...
			if tryb == 0:
				getPage(self.url.encode('utf-8')).addCallback(self.versionInfoCallBack, 0).addErrback(self.errorUpdate)
			else:
				getPage(self.url.encode('utf-8')).addCallback(self.versionInfoCallBack, 1).addErrback(self.errorUpdate)
		...
		try:
			...
			for path in paths:
				if " = " in path:
					...
					if key == "Version": self.newVersion = val.replace('"', "").strip()
					if key == "File": self.newFile = val.replace('"', "").strip()
					if key == "Url": self.fileUrl = val.replace('"', "").strip()
					if key == "MD5": self.MD5 = val.replace('"', "").strip()
