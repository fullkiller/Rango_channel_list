...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		...
		safe_tar_extract('/tmp/update_rango.tar.gz', '/', ['/usr/lib/enigma2/python/Plugins/Extensions/RangoPolishChannelsUpdater/'])
		if os.path.exists('/tmp/update_rango.tar.gz'):
			os.remove('/tmp/update_rango.tar.gz')
		os.system('sync')
