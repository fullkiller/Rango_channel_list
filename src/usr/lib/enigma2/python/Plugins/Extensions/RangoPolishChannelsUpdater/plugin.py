...
from .utils import reloadChannelList, CommonChannelListScreen, Djcrash, CommonPiconListScreen, safe_tar_extract, remove_wildcard
from . import _
import os
...
class ChannelListUpdateMenu(Screen):
	...
	def versionInfoCallBackQuiet(self, html):
		...
		try:
			cur_v = int(version)
			new_v = int(self.newVersion)
		except:
			cur_v = 0
			new_v = 0
		print("Current Version : ", cur_v, " Server Version : ", new_v) #FIXED by acypaczom    print "Current Version : ", int(version), " Server Version : ", self.newVersion
		...
		if new_v > cur_v:
			...
			version,
			_('Aktualizacja jest dostępna!\n\nCzy chcesz uruchomic aktualizację teraz?'))
	...
	def versionInfoCallBack(self, html):
		...
		try:
			...
			cur_v = 0
			new_v = 0
		print("Current Version : ", cur_v, " Server Version : ", new_v) #FIXED acypaczom  print "Current Version : ", int(version), " Server Version : ", self.newVersion
		self.updateurl = self.fileUrl + "/" + self.newFile
...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		...
		try:
			...
			print("[ChannelListUpdateUpdater] error extracting update:", e)
