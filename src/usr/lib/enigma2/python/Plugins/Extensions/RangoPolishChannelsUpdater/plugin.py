...
class ChannelListUpdateMenu(Screen):
	...
	def __init__(self, session):
		...
		self["info"] = Label(_("Plugin do aktualizacji list kanałow tworzonych przez HSWG, DjCrash'a, Krzyśka80 ,fullkiller™ i @Seto. Bez ich pracy nie byłoby list kanałów.\n\nRango - %s") % version)
	...
	def versionInfoCallBackQuiet(self, html):
		...
		if self.fileUrl and self.newFile:
			self.updateurl = self.fileUrl + ("/" if not self.fileUrl.endswith("/") else "") + self.newFile
