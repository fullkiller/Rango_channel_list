...
class ChannelListUpdateMenu(Screen):
	...
	def check_updates(self, tryb = 1):
		try:
			...
			getPage(self.url.encode('utf-8')).addCallback(self.versionInfoCallBack, tryb).addErrback(self.errorUpdate)
		except Exception as error:
			print(error)

	def versionInfoCallBack(self, html, tryb):
		try:
...
class K80(CommonChannelListScreen):
	def __init__(self, session, args = None):
		...
		self.url1 = 'https://raw.githubusercontent.com/fullkiller/Rango_channel_list/main/latest5x.txt'
...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		if self.expected_md5 and self.expected_md5 != '00000000000000000000000000000000':
			self['status'].setText(_('Weryfikacja MD5...'))
			md5_hash = hashlib.md5()
			with open(self.dlfile, "rb") as f:
				for byte_block in iter(lambda: f.read(4096), b""):
					md5_hash.update(byte_block)
			if md5_hash.hexdigest() != self.expected_md5:
				self['status'].setText(_('Błąd MD5! Pobieranie nieudane.'))
