...
from .utils import reloadChannelList, CommonChannelListScreen, Djcrash, CommonPiconListScreen, decode_html, verify_md5, remove_wildcard
...
class ChannelListUpdateMenu(Screen):
	...
	def __init__(self, session):
		...
		self["info"] = Label("Plugin do aktualizacji list kanałów tworzonych przez HSWG, DjCrash'a, Krzyśka80, fullkiller™ i @Seto. Bez ich pracy nie byłoby list kanałów.\n\nRango - %s" % version)
	...
	def updateList(self):
		...
		images = [
			("hswg.png", "Pobierz listę od HSWG_(bzyk83)", "Listy dla HotBird 13.0E oraz Astra 19.2E", "hswg"),
			("djcrash.png", "Pobierz listę od DjCrasha", "Listy dla HotBird 13.0E, 19.2E, 23,5E, 28,2E oraz obrotnica", "djcrash"),
			("k80.png", "Pobierz listę od Krzysiek80", "Listy dla HotBird 13.0E", "k80"),
			("fk.png", "Pobierz listę od fullkiller™", "Listy dla HotBird 13.0E", "fk"),
			("ms.png", "Pobierz config mulistalker", "MAC links", "master"),
			("seto.png", "Pobierz listę od @Seto", "Listy dla HotBird 13.0E oraz Astra 19.2E", "seto"),
			("picon.png", "Pobierz Picony od @Seto i RJ", "Picony dla HotBird 13.0E oraz Astra 19.2E", "masterpicon"),
		]

		for img, name, desc, idx in images:
			mypixmap = mypath + img
			if os.path.exists(mypixmap):
				png = LoadPixmap(mypixmap)
				res = (_(name), png, idx, _(desc))
	...
	def versionInfoCallBackQuiet(self, html):
		...
		print("Current Version : ", version, " Server Version : ", self.newVersion)
		self.updateurl = self.fileUrl + self.newFile if self.fileUrl.endswith('/') else self.fileUrl + "/" + self.newFile
...
class ChannelListUpdateUpdater(Screen):
	def __init__(self, session, updateurl, expected_md5=""):
	...
	def downloadFinished(self, string = ''):
		if not verify_md5(self.dlfile, self.expected_md5):
			self['status'].setText(_('Błąd MD5 pliku aktualizacji!'))
			return
...
def Plugins(**kwargs):
	...
	if screenwidth == 1920:
		return [
			...
			PluginDescriptor(name = "Polskie listy kanałów", description=_('Najnowsza kolekcja polskich list kanałów'), icon='images/pluginhd.png', where=PluginDescriptor.WHERE_PLUGINMENU, fnc=main)
		]
	else:
		return [
			PluginDescriptor(name = "Polskie listy kanałów", description=_('Najnowsza kolekcja polskich list kanałów'), where = PluginDescriptor.WHERE_MENU, fnc = menu),
			PluginDescriptor(name = "Polskie listy kanałów", description=_('Najnowsza kolekcja polskich list kanałów'), icon='images/plugin.png', where=PluginDescriptor.WHERE_PLUGINMENU, fnc=main)
		]
