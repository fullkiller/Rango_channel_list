...
class ChannelListUpdateMenu(Screen):
	...
	def check_updates(self, tryb = 1):
		try:
			...
			getPage(self.url.encode('utf-8') if isinstance(self.url, str) else self.url).addCallback(self.versionInfoCallBack, tryb).addErrback(self.errorUpdate)
		except Exception as error:
			print(error) #print str(error) fixed by acypaczom  python 2.6 and 3 compatible

	def versionInfoCallBack(self, html, tryb=1):
		...
		for path in paths:
			if ' = ' in path:
				...
				if key == "Version":
					...
					self.newFile = val.replace('"', "").strip()
				elif key == "Url":
					self.fileUrl = val.replace('"', "").strip()
				elif key == "MD5":
					self.MD5 = val.replace('"', "").strip()
...
class ChannelListUpdateUpdater(Screen):
	...
	def startUpdate(self):
		...
		self.download = downloadWithProgress(self.updateurl.encode('utf-8') if isinstance(self.updateurl, str) else self.updateurl, self.dlfile.encode('utf-8') if isinstance(self.dlfile, str) else self.dlfile)
	...
	def downloadFinished(self, string = ''):
		...
		try:
			...
			self['status'].setText(_('Błąd podczas rozpakowywania aktualizacji!'))
