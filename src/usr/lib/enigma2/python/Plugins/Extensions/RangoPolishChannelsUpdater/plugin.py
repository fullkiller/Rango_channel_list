...
from .utils import reloadChannelList, CommonChannelListScreen, Djcrash, CommonPiconListScreen, safe_tar_extract, remove_wildcard, decode_html
from . import _
...
class ChannelListUpdateMenu(Screen):
	...
	def updateList(self):
		...
		for img, name, desc, idx in options:
	...
	def parseVersion(self, html):
		...
		self.newVersion = info.get("Version", "0")
		self.newFile = info.get("File", "")
		self.fileUrl = info.get("Url", "")
		self.MD5 = info.get("MD5", "0")
