...
class ChannelListUpdateUpdater(Screen):
	...
	def startUpdate(self):
		...
		if "raw.githubusercontent.com" in self.updateurl:
			self.updateurl = self.updateurl.replace("http://", "https://")
	...
	def downloadFinished(self, string = ''):
		if self.md5 and self.md5 != "00000000000000000000000000000000":
			try:
				...
				print("[Rango] MD5 check failed: ", e)
				self['status'].setText(_('Błąd sprawdzania MD5!'))
				return
		...
		try:
			...
			print("[Rango] Update extraction failed: ", e)
