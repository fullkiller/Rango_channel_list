...
import os, tarfile, hashlib, shutil, glob
...
class ChannelListUpdateMenu(Screen):
	...
	def __init__(self, session):
		...
		self["info"] = Label(_("Plugin do aktualizacji list kanałów tworzonych przez HSWG, DjCrash'a, Krzyśka80 ,fullkiller™ i @Seto. Bez ich pracy nie było by list kanałów.\n\nRango - %s") % version)
	...
	def versionInfoCallBack(self, html):
		try:
			self.parseVersion(html)
			if int(self.newVersion) == int(version):
				message = '%s %s\n%s %s\n\n%s' % (_('Wersja na Serwerze:'), self.newVersion, _('Wersja zainstalowana:'), version, _('Posiadasz aktualna wersje pluginu !'))
...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		...
		try:
			with tarfile.open(self.dlfile, "r:gz") as tar:
...
def Plugins(**kwargs):
	...
	return [
		...
		PluginDescriptor(name = _("Polskie listy kanałów"), description=_('Najnowsza kolekcja polskich list kanałów'), icon=icon, where=PluginDescriptor.WHERE_PLUGINMENU, fnc=main)
	]
