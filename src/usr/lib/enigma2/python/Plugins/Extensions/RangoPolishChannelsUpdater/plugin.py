...
class ChannelListUpdateMenu(Screen):
	...
	def check_updates(self, tryb = 1):
		try:
			self.url = 'http://fullkiller.ugu.pl/RangoPolskieListyKanalow/soft/version.txt'.encode('utf-8')
			#self.url = 'https://raw.githubusercontent.com/fullkiller/Rango_channel_list/version.txt'.encode('utf-8')
			if tryb == 0:
	...
	def downloadFinished(self, string = ''):
		...
		if self.md5 and self.md5 != "00000000000000000000000000000000":
			...
			if file_hash != self.md5:
				self['status'].setText(_('Błąd sumy kontrolnej MD5! Przerwano instalację.'))
				return
