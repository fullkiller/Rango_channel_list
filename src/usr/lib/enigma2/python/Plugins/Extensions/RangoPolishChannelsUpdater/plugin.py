...
from .utils import reloadChannelList, CommonChannelListScreen, Djcrash, CommonPiconListScreen, safe_tar_extract, os_sync
from . import _
import os
...
class ChannelListUpdateMenu(Screen):
	...
	def __init__(self, session):
		...
		self["actions"] = ActionMap(["WizardActions", "ColorActions"], {"red": lambda: self.check_updates(1), "ok": self.KeyOk, "back": self.close})
	...
	def check_updates(self, tryb = 1):
		try:
			...
			getPage(self.url).addCallback(self.versionInfoCallBack, quiet=(tryb == 0)).addErrback(self.errorUpdate)
	...
	def versionInfoCallBack(self, html, quiet=False):
		try:
			...
			if '[plugin]' not in html:
				html = '[plugin]\n' + html
			config.read_file(io.StringIO(html))

			self.newVersion = config.get('plugin', 'Version').replace('"', "").strip()
			self.newFile = config.get('plugin', 'File').replace('"', "").strip()
			self.fileUrl = config.get('plugin', 'Url').replace('"', "").strip()
			self.MD5 = config.get('plugin', 'MD5').replace('"', "").strip()
		except Exception:
			# Fallback for old format
			content = str(html)
			paths = content.split("\n")
			for path in paths:
				if "Version =" in path: self.newVersion = path.split("=")[1].replace('"', "").strip()
				if "File =" in path: self.newFile = path.split("=")[1].replace('"', "").strip()
				if "Url =" in path: self.fileUrl = path.split("=")[1].replace('"', "").strip()
				if "MD5 =" in path: self.MD5 = path.split("=")[1].replace('"', "").strip()
...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		...
		try:
			...
			print("Update extraction failed: %s" % e)
