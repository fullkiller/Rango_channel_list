	...
	def parseVersionInfo(self, html):
		...
		try:
			...
			if cp.has_section("plugin"):
				self.newVersion = cp.get("plugin", "Version").replace('"', "").strip()
				self.newFile = cp.get("plugin", "File").replace('"', "").strip()
				self.fileUrl = cp.get("plugin", "Url").replace('"', "").strip()
				self.MD5 = cp.get("plugin", "MD5").replace('"', "").strip()
		except Exception as e:
			print(f"Error parsing version.txt: {e}")
