...
def safe_tar_extract(tar_file, path=".", members=None, **kwargs):
	try:
		with tarfile.open(tar_file) as tar:
			for member in tar.getmembers():
				member_path = os.path.join(path, member.name)
				if not is_safe_path(path, member_path):
					print("[ChannelListUpdate] Attempted Path Traversal in Tar File: %s" % member.name)
					return False
			tar.extractall(path, members, **kwargs)
		return True
	except Exception as e:
		print("[ChannelListUpdate] Error extracting tar: %s" % str(e))
		return False

def safe_zip_extract(zip_file, path=".", members=None, pwd=None):
	try:
		with zipfile.ZipFile(zip_file) as zf:
			for member in zf.infolist():
				member_path = os.path.join(path, member.filename)
				if not is_safe_path(path, member_path):
					print("[ChannelListUpdate] Attempted Path Traversal in Zip File: %s" % member.filename)
					return False
...
class CommonChannelListScreen(Screen):
	skin = """
		<screen position="center,center" size="1150,340" >
			...
			<widget name="sats3" 		position="900,90" size="190,25" font="Regular;18" halign="left" />
	...
	def yellow(self):
		...
		try:
			if self.url3x != '' and self.canDownload == 1:
				self["status"].setText('Status: Pobieranie listy')
				downloadPage(self.url3x, '/tmp/chList4x1.tgz').addCallback(self.downloadCallBack, "/tmp/chList4x1.tgz").addErrback(self.errorUpdate)
		except:
			self["status"].setText('Status: Nie mogę się połączyć z serwerem !')
			self.reloadUrlList(self.url1, self.url2, self.url3)
	...
	def reloadUrlList(self, url1, url2, url3=None):
		...
		if self.url3:
			getPage(self.url3).addCallback(self.versionInfoCallBack, "3x").addErrback(self.errorUpdate)
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			for f in ["*.tv", "*.radio", "lamedb", "satellites.xml", "whitelist", "blacklist", "revision"]:
				os.system('rm -f /etc/enigma2/%s' % f)
	...
	def errorUpdate(self, html):
		...
		self["newVersion3x"].setText("Błąd")
...
class Djcrash(Screen):
	...
	def installChList(self, html):
		...
		if safe_zip_extract(r"/tmp/lista.zip", r"/tmp/chList/"):
			...
			src_path = '/tmp/chList/list_by_djcrash-e2_' + self.chooseList + '/'
			if os.path.exists(src_path):
				...
				self["status"].setText('Status: Błąd instalacji - brak plików !')
		else:
			self["status"].setText('Status: Błąd archiwum !')
...
class CommonPiconListScreen(Screen):
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			if not os.path.exists(self.piconLocationToInstall):
				os.makedirs(self.piconLocationToInstall)
