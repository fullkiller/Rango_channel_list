...
def is_path_safe(base, target):
	...
	if base == '/':
		return True
	return target.startswith(base + os.path.sep)

def safe_tar_extract(archive, path):
	with tarfile.open(archive, 'r:gz') as t:
		for member in t.getmembers():
			if not is_path_safe(path, os.path.join(path, member.name)):
				raise Exception("Potential path traversal attempt: %s" % member.name)
		t.extractall(path)
	...
	with zipfile.ZipFile(archive, 'r') as z:
		for member in z.namelist():
			if not is_path_safe(path, os.path.join(path, member)):
				raise Exception("Potential path traversal attempt: %s" % member)
		z.extractall(path)
