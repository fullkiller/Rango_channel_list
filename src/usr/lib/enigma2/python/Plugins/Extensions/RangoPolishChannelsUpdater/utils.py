...
def safe_tar_extract(tar_file, dest_path):
	if not os.path.exists(dest_path):
		os.makedirs(dest_path)
	with tarfile.open(tar_file, 'r:gz') as tar:
		for member in tar.getmembers():
			member_path = os.path.join(dest_path, member.name)
			if is_path_safe(dest_path, member_path):
				tar.extract(member, dest_path)
			else:
				print("[ChannelListUpdate] Skipping unsafe file: %s" % member.name)

def safe_zip_extract(zip_file, dest_path):
	if not os.path.exists(dest_path):
		os.makedirs(dest_path)
	with zipfile.ZipFile(zip_file, 'r') as zf:
		for member in zf.infolist():
			...
			if is_path_safe(dest_path, member_path):
				...
				print("[ChannelListUpdate] Skipping unsafe file: %s" % member.filename)
