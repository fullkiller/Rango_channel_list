		...
		with tarfile.open(tar_file, 'r:gz') as tar:
			for member in tar.getmembers():
				if not is_path_safe(target_path, os.path.join(target_path, member.name)):
					print("[ChannelListUpdate] Security Warning: Path traversal detected in tar archive!")
					return False
	...
	try:
		with zipfile.ZipFile(zip_file, 'r') as zip_ref:
			for member in zip_ref.namelist():
				if not is_path_safe(target_path, os.path.join(target_path, member)):
					print("[ChannelListUpdate] Security Warning: Path traversal detected in zip archive!")
					return False
...
def verify_md5(filepath, expected_md5):
	if not expected_md5 or expected_md5 == "0" * 32:
		return True
...
class Djcrash(Screen):
	...
	def downloadFromUpload(self, failure=None):
		...
		downloadPage(url.encode('utf-8'), '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromPKT_once)
	...
	def downloadFromPKT_once(self, failure=None):
		...
		downloadPage(url.encode('utf-8'), '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFailed)

	def downloadFromPKT(self, failure=None):
		self["status"].setText("Status: Alternatywne pobieranie listy " + self.chooseList)
		url = "http://addon.pkteam.pl/channel_lists_e2/enigma2_list_by_djcrash-" + self.newVersion + ".zip"
		downloadPage(url.encode('utf-8'), '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromUpload_once)
	...
	def installChList(self, html):
		self["status"].setText('Status: Rozpakowywanie')
		if safe_zip_extract("/tmp/lista.zip", "/tmp/chList/"):
