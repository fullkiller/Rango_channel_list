...
def safe_tar_extract(filepath, target_path):
		...
		with tarfile.open(filepath, "r:gz") as t:
			for member in t.getmembers():
				...
				if not is_path_safe(target_path, member_path):
					print("[ChannelListUpdate] unsafe tar member: %s" % member.name)
					return False
...
def safe_zip_extract(filepath, target_path):
	if not is_path_safe("/", target_path):
		return False
	try:
		with zipfile.ZipFile(filepath, 'r') as zf:
			for member in zf.namelist():
				...
				if not is_path_safe(target_path, member_path):
					print("[ChannelListUpdate] unsafe zip member: %s" % member)
					return False
...
class Djcrash(Screen):
	...
	def getNewChListVersion(self, html):
		try:
			with open("/tmp/revision", 'r') as f:
				for line in f:
					if line.startswith("DATE="):
						self.newVersion = line.split("=")[1].strip().replace('"', "")
					elif line.startswith("VER="):
						self.chID = line.split("=")[1].strip().replace('"', "")
			...
			print("[ChannelListUpdate] Error parsing revision: %s" % str(e))
	...
	def downloadFromUpload(self, failure):
...
class CommonPiconListScreen(Screen):
	...
	def choiceBoxPiconLocation(self):
		...
		if os.path.exists('/proc/mounts'):
			with open('/proc/mounts', 'r') as f:
				for line in f:
					...
					if len(parts) > 1:
						mount_point = parts[1]
						if mount_point in ['/media/cf', '/media/usb', '/media/usb2', '/media/usb3', '/media/card', '/media/hdd', '/media/hdd2']:
		...
		for m in mounts:
			...
			if m in ['/media/usb', '/media/usb2', '/media/usb3', '/media/hdd', '/media/hdd2']:
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			for p in ["ZZPiconGLASS", "ZZPiconBLACK", "ZZPiconRJ", "ZZPiconChristmas"]:
				if self.piconTypeToInstall == p:
					dest = dest.replace(p, "ZZPicon")
