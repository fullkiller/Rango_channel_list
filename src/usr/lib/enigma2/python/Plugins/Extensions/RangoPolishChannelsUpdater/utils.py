...
def is_path_safe(base, path, allowed_paths=None):
...
def safe_tar_extract(tar_file, path):
	allowed = ['/etc/enigma2/', '/usr/share/enigma2/', '/usr/lib/enigma2/python/Plugins/Extensions/RangoPolishChannelsUpdater/', '/picon/', '/media/hdd/', '/media/usb/', '/media/cf/', '/media/card/']
	try:
		with tarfile.open(tar_file, 'r:gz') as tar:
			for member in tar.getmembers():
				if not is_path_safe(path, os.path.join(path, member.name), allowed if path == '/' else None):
					print("[ChannelListUpdate] Skipping unsafe path in tar: %s" % member.name)
					continue
	...
	try:
		with zipfile.ZipFile(zip_file, 'r') as zip_ref:
			for member in zip_ref.namelist():
				if not is_path_safe(path, os.path.join(path, member), allowed if path == '/' else None):
					print("[ChannelListUpdate] Skipping unsafe path in zip: %s" % member)
					continue
...
def remove_wildcard(pattern):
	for f in glob.glob(pattern):
		try:
			...
			print("[ChannelListUpdate] Error removing %s: %s" % (f, str(e)))
...
class CommonChannelListScreen(Screen):
	...
	def red(self):
		...
		try:
			...
			# self.reloadUrlList(self.url1, self.url2) # infinite loop risk
	...
	def versionInfoCallBack(self, html, ltype="1x"):
		try:
			html_decoded = decode_html(html)
			new = dict([ l.split(':',1) for l in html_decoded.split("\n") if l.find(":") > 0])
			if 'version' in new:
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			...
			if safe_tar_extract(plik, "/"):
				...
				self["status"].setText('Status: Błąd podczas wypakowywania !')
			os.remove(plik)
		except:
...
class Djcrash(Screen):
	...
	def getNewChListVersion(self, html):
		...
		paths = content.split("\n")
		for path in paths:
			if " = " in path: # for safety
				pass
			if "=" in path:
				key, value = path.split("=", 1)
				if key == "DATE":
					self.newVersion = value.replace('"', "")
				if key == "VER":
					self.chID = value.replace('"', "")
		file.close()
	...
	def installChList(self, html):
		self["status"].setText('Status: Rozpakowywanie')
		if not safe_zip_extract(r"/tmp/lista.zip", r"/tmp/chList/"):
			self["status"].setText('Status: Błąd archiwum !')
			return
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			if safe_tar_extract(plik, self.piconLocationToInstall):
				...
				self["status"].setText('Status: Błąd podczas instalacji piconów !')
			os.remove(plik)
			self.askRestart()
		except:
			self["status"].setText('Status: Błąd archiwum !')
