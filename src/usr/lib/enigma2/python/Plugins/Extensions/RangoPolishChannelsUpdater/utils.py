...
def safe_zip_extract(zip_file, path=".", members=None, pwd=None):
	for name in zip_file.namelist():
		if not os.path.abspath(os.path.join(path, name)).startswith(os.path.abspath(path)):
			raise Exception("Attempted Path Traversal in Zip File")
	zip_file.extractall(path, members, pwd)
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			for f in ["*.tv", "*.radio", "lamedb", "satellites.xml", "whitelist", "blacklist", "revision"]:
				for path in glob.glob("/etc/enigma2/" + f):
...
class Djcrash(Screen):
	...
	def installChList(self, html):
		...
		try:
			...
			print(error)
			self["status"].setText(_('Status: Błąd instalacji !'))
...
class CommonPiconListScreen(Screen):
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			for ptype in ["ZZPiconGLASS", "ZZPiconBLACK", "ZZPiconRJ", "ZZPiconChristmas"]:
				if self.piconTypeToInstall == ptype:
					self.piconLocationToInstall = self.piconLocationToInstall.replace(ptype, "ZZPicon")
