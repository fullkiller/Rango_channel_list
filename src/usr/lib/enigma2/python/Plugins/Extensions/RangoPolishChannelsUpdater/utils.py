...
def safe_zip_extract(zf, path=".", members=None, pwd=None):
...
class Djcrash(Screen):
	...
	def reloadUrlList(self):
		downloadPage("https://addon.pkteam.pl/channel_lists_e2/revision".encode('utf-8'), b'/tmp/revision').addCallback(self.getNewChListVersion).addErrback(self.errorUpdate)

	def downloadFromUpload(self, failure_instance=None):
		self["status"].setText("Status: Pobieranie listy " + self.chooseList)
		downloadPage(("https://upload.sat-elita.net.pl/download.php?id=" + self.chID).encode('utf-8'), b'/tmp/lista.zip').addCallback(self.installChList).addErrback(self.errorUpdate)

	def downloadFromPKT(self, failure_instance=None):
		self["status"].setText("Status: Alternatywne pobieranie listy " + self.chooseList)
		downloadPage(("https://addon.pkteam.pl/channel_lists_e2/enigma2_list_by_djcrash-" + self.newVersion + ".zip").encode('utf-8'), b'/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromUpload)
