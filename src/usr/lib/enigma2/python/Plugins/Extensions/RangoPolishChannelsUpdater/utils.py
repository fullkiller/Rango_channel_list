...
def is_path_safe(base, target):
	...
	if base == "/":
		return True
	base_with_sep = base if base.endswith(os.path.sep) else base + os.path.sep
	return os.path.commonprefix([base_with_sep, target + os.path.sep]) == base_with_sep

def safe_tar_extract(tar_file, path):
	with tarfile.open(tar_file) as tar:
		for member in tar.getmembers():
			if not is_path_safe(path, os.path.join(path, member.name)):
				print("[ChannelListUpdate] Security alert: attempted path traversal in tar: %s" % member.name)
				continue
	...
	with zipfile.ZipFile(zip_file, 'r') as zip_ref:
		for member in zip_ref.namelist():
			if not is_path_safe(path, os.path.join(path, member)):
				print("[ChannelListUpdate] Security alert: attempted path traversal in zip: %s" % member)
				continue
