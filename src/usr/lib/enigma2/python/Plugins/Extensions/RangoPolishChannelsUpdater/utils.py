...
def safe_tar_extract(tar_file, path=".", **kwargs):
	try:
		...
		print("[safe_tar_extract] Error: %s" % str(e))
		return False
...
def safe_zip_extract(zip_file, path="."):
	try:
		...
		print("[safe_zip_extract] Error: %s" % str(e))
		return False
...
class CommonChannelListScreen(Screen):
	skin = """
		<screen position="center,center" size="1150,340" >
			...
			<eLabel 			position="880,63" size="2,135" font="Regular;18" halign="center" backgroundColor="white" />
			...
			<widget name="key_yellow"	position="870,230" size="280,40" font="Regular;24" halign="center" valign="center" backgroundColor="yellow" />
	...
	def __init__(self, session, args = None):
		...
		self["sats2x"] = Label("HB 13.0E & Astra 19.2E")
		self["sats3"] = Label("4x1")
		...
		self["actions"] = ActionMap(["OkCancelActions","ColorActions"], {"red": self.red, "green": self.green, "yellow": self.yellow, "cancel": self.close , "ok": self.close}, -1)
	...
	def yellow(self):
		self["status"].show()
		try:
			if self.url3x != "" and self.canDownload == 1:
				self["status"].setText(_('Status: Pobieranie listy'))
				downloadPage(self.url3x.encode('utf-8'), '/tmp/chList3x1.tgz').addCallback(self.downloadCallBack, "/tmp/chList3x1.tgz").addErrback(self.errorUpdate)
		except:
			self["status"].setText(_('Status: Nie mogę się połączyć z serwerem !'))
			self.reloadUrlList(self.url1, self.url2, self.url3)
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			for f in ["/etc/enigma2/*.tv", "/etc/enigma2/*.radio", "/etc/enigma2/lamedb", "/etc/enigma2/satellites.xml", "/etc/enigma2/whitelist", "/etc/enigma2/blacklist", "/etc/enigma2/revision"]:
				for file_to_remove in glob.glob(f):
					try:
	...
	def errorUpdate(self, html):
		...
		if "newVersion3x" in self:
			self["newVersion3x"].setText(_("Błąd"))
...
class Djcrash(Screen):
	...
	def installChList(self, html):
		self["status"].setText(_('Status: Rozpakowywanie'))
		if safe_zip_extract(r"/tmp/lista.zip", r"/tmp/chList/"):
			...
			source_dir = '/tmp/chList/list_by_djcrash-e2_' + self.chooseList
...
class CommonPiconListScreen(Screen):
	...
	def choiceBoxPiconLocation(self):
		...
		if mycf:
			menu.append((_("Compact Flash (/media/cf/%s/picon)") % self.piconTypeToInstall, mycf + "/" + self.piconTypeToInstall))
		if myusb:
		...
		if mysd:
			menu.append((_("SD Card (/media/card/%s/picon)") % self.piconTypeToInstall, mysd+ "/" + self.piconTypeToInstall))
		if myhdd:
		...
		try:
			...
			if 'version' in new:
				...
				if ltype == "1x":
					...
					self.url2x = new.get('url', "")
					self.newVersion2 = new['version']
				elif ltype == "3x":
					self.url3x = new.get('url', "")
					self.newVersion3 = new['version']
				self.canDownload = 1
		except Exception as error:
			print(error)
