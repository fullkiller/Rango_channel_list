...
class CommonPiconListScreen(Screen):
		...
		for path, name in possible_paths:
			if path in mounts:
				...
				if name in ('USB-1', 'USB-2', 'USB-3', 'HDD', 'HDD2'):
					menu.append((_("%s (%s/picon)") % (name, path), path))
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			for t_type in ["ZZPiconGLASS", "ZZPiconBLACK", "ZZPiconRJ", "ZZPiconChristmas"]:
				if self.piconTypeToInstall == t_type:
					dest = dest.replace(t_type, "ZZPicon")
			for t_type in ["XPiconsBLACK", "SetoXPicons"]:
				if self.piconTypeToInstall == t_type:
					dest = dest.replace(t_type, "XPicons")
...
class ChannelListDownloadTask(Task):
	...
	def run(self, callback):
		...
		url_to_download = self.url.encode('utf-8') if isinstance(self.url, str) else self.url
		self.download = downloadWithProgress(url_to_download, self.path)
