...
import traceback
import re, os, tarfile, zipfile, shutil, sys, glob
...
def safe_tar_extract(tar_file, path):
	...
	def safe_extract(tar, path=".", members=None, **kwargs):
		numeric_owner = kwargs.get('numeric_owner', False)
...
class CommonChannelListScreen(Screen):
	skin = """
		<screen position="center,center" size="860,340" >
			...
			<widget name="newVersion3x" 	position="600,180" size="290,25" font="Regular;18" halign="left" />
...
class Djcrash(Screen):
	...
	def downloadFromUpload(self):
		...
		downloadPage(("http://upload.sat-elita.net.pl/download.php?id=" + self.chID).encode('utf-8'), b'/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromPKT)
	...
	def installChList(self, html):
		self["status"].setText('Status: Rozpakowywanie')
		try:
			if not os.path.exists("/tmp/chList"):
...
class CommonPiconListScreen(Screen):
	...
	def prepareDownload(self):
		...
		print ("==============================")
		self.finalUrl = self.baseUrl.rstrip('/') + "/" + self.piconPositionToInstall + "/" + self.piconTypeToInstall + "/"+ self.piconTypeToInstall + self.piconPositionToInstall + ".tar.gz"
