...
def is_path_safe(base, path):
	...
	if base == '/':
		allowed = ['/usr', '/etc', '/media', '/tmp', '/var', '/hdd']
		return any(path.startswith(a + os.sep) or path == a for a in allowed)
...
def safe_tar_extract(tar, path='.', **kwargs):
	for member in tar.getmembers():
		...
		if not is_path_safe(path, member_path):
			print("[ChannelListUpdate] Security alert: path traversal detected in tar!")
			continue
		tar.extract(member, path, **kwargs)

def safe_zip_extract(zf, path='.'):
	for member in zf.namelist():
		member_path = os.path.join(path, member)
		if not is_path_safe(path, member_path):
			print("[ChannelListUpdate] Security alert: path traversal detected in zip!")
			continue
...
class CommonChannelListScreen(Screen):
	...
	def __init__(self, session, args = None):
		...
		self.newVersion1 = 'unknown'
		self.newVersion2 = 'unknown'
		self.url1x = ''
		self.url2x = ''
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			for pattern in ['*.tv', '*.radio', 'lamedb', 'satellites.xml', 'whitelist', 'blacklist', 'revision']:
...
class Djcrash(Screen):
	...
	def downloadFromUpload(self):
		...
		downloadPage(url.encode('utf-8'), '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromPKT)
	...
	def downloadFromPKT(self):
		...
		downloadPage(url.encode('utf-8'), '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromUpload)
...
class CommonPiconListScreen(Screen):
	skin = """
		<screen position="center,center" size="1150,340" >
			...
			<widget name="key_red"		position="270,230" size="280,40" font="Regular;24" halign="center" valign="center" backgroundColor="red" />
			<widget name="key_green"	position="570,230" size="280,40" font="Regular;24" halign="center" valign="center" backgroundColor="green" />
			<widget name="key_yellow"	position="870,230" size="270,40" font="Regular;24" halign="center" valign="center" backgroundColor="yellow" />
	...
	def __init__(self, session, args = None):
		...
		self["sats3"] = Label("DVB-T2/Mobilne")
