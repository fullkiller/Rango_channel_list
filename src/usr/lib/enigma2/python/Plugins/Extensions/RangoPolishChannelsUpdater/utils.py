...
def safe_tar_extract(archive, dest_dir, **kwargs):
	with tarfile.open(archive, 'r:gz') as t:
		for member in t.getmembers():
			member_path = os.path.join(dest_dir, member.name)
			if not is_path_safe(dest_dir, member_path):
				raise Exception("Unsafe path in archive: %s" % member.name)
		t.extractall(dest_dir, **kwargs)

def safe_zip_extract(archive, dest_dir):
	with zipfile.ZipFile(archive, 'r') as z:
		for member in z.namelist():
			member_path = os.path.join(dest_dir, member)
			if not is_path_safe(dest_dir, member_path):
				raise Exception("Unsafe path in archive: %s" % member)
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			...
			for f in ["*.tv", "*.radio", "lamedb", "satellites.xml", "whitelist", "blacklist", "revision"]:
				for filepath in [os.path.join("/etc/enigma2/", f)]:
					import glob
					for found in glob.glob(filepath):
...
class Djcrash(Screen):
	...
	def getCurrentChListVersion(self):
		try:
			with open("/etc/enigma2/revision", 'r') as f:
				...
				for path in paths:
					if "=" in path:
						self.curVersion = path.split("=")[1].strip().replace('"', '')
		except:
	...
	def installChList(self, html):
		...
		try:
			...
			safe_zip_extract("/tmp/lista.zip", temp_dir)
		except Exception as e:
			print(e)
		...
		try:
			import glob
			for f in ["*.tv", "*.radio", "lamedb", "satellites.xml", "whitelist", "blacklist", "revision"]:
				for found in glob.glob(os.path.join("/etc/enigma2/", f)):
					os.remove(found)

			src_path = os.path.join(temp_dir, 'list_by_djcrash-e2_' + self.chooseList)
			...
			shutil.copy2(os.path.join(temp_dir, 'revision'), '/etc/enigma2/revision')
...
class CommonPiconListScreen(Screen):
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			picon_path = os.path.join(loc, "picon")
