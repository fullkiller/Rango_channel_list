...
def is_path_safe(base, target, allowed_prefixes=None):
	...
	if base == '/':
		if allowed_prefixes is None:
			allowed_prefixes = ['/etc/enigma2/', '/usr/share/enigma2/', os.path.dirname(__file__)]
...
def safe_tar_extract(tar_file, target_path):
	with tarfile.open(tar_file, 'r:gz') as tar:
		for member in tar.getmembers():
			if is_path_safe(target_path, os.path.join(target_path, member.name)):
				tar.extract(member, target_path)

def safe_zip_extract(zip_file, target_path):
	with zipfile.ZipFile(zip_file, 'r') as zip_ref:
		for member in zip_ref.infolist():
			if is_path_safe(target_path, os.path.join(target_path, member.filename)):
...
def verify_md5(filepath, expected_md5):
	if expected_md5 == "000000000000000000000000" or expected_md5 == "":
		return True
			...
			with open("/etc/enigma2/revision", 'r') as f:
				...
				for path in paths:
					if path.startswith('DATE='):
						self.curVersion = path.split("=")[1].replace('"', "").strip()
		except:
...
class CommonPiconListScreen(Screen):
	...
	def choiceBoxPiconLocation(self):
		...
		# Sort mounts to check more specific paths first
		for m in sorted(mounts, key=len, reverse=True):
			if m.startswith('/media/') or m == '/hdd' or m.startswith('/mnt/'):
				display_name = m.split('/')[-1].upper()
				menu.append((_("%s (%s/%s/picon)") % (display_name, m, self.piconTypeToInstall), m + "/" + self.piconTypeToInstall))
				menu.append((_("%s (%s/picon)") % (display_name, m), m))
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			for t in ["ZZPiconGLASS", "ZZPiconBLACK", "ZZPiconRJ", "ZZPiconChristmas"]:
				if self.piconTypeToInstall == t:
					loc = loc.replace(t, "ZZPicon")
			for t in ["XPiconsBLACK", "SetoXPicons"]:
				if self.piconTypeToInstall == t:
					loc = loc.replace(t, "XPicons")
