...
def safe_tar_extract(tar, path=".", members=None, **kwargs):
...
def safe_zip_extract(zf, path=".", members=None, **kwargs):
    for member in zf.infolist():
        member_path = os.path.join(path, member.filename)
        if not os.path.abspath(member_path).startswith(os.path.abspath(path)):
            raise Exception("Attempted Path Traversal in Zip File")
    zf.extractall(path, members, **kwargs)
...
class Djcrash(Screen):
	...
	def reloadUrlList(self):
		getPage("http://addon.pkteam.pl/channel_lists_e2/revision").addCallback(self.getNewChListVersion).addErrback(self.errorUpdate)

	def downloadFromUpload(self, failure=None):
		self["status"].setText("Status: Pobieranie listy " + self.chooseList)
		downloadPage("http://upload.sat-elita.net.pl/download.php?id=" + self.chID, '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.errorFinal)
	...
	def errorFinal(self, failure):
		self["status"].setText("Status: Błąd pobierania ze wszystkich źródeł!")
...
class CommonPiconListScreen(Screen):
	skin = """
		<screen position="center,center" size="1150,340" >
			...
			<widget name="newVersion1x" 	position="300,150" size="190,25" font="Regular;18" halign="left" />
			<widget name="newVersion2x" 	position="600,150" size="290,25" font="Regular;18" halign="left" />
			<widget name="newVersion3x" 	position="900,150" size="190,25" font="Regular;18" halign="left" />
			...
			<widget name="key_red"		position="30,230" size="280,40" font="Regular;24" halign="center" valign="center" backgroundColor="red" />
			<widget name="key_green"	position="330,230" size="280,40" font="Regular;24" halign="center" valign="center" backgroundColor="green" />
			<widget name="key_yellow"	position="630,230" size="280,40" font="Regular;24" halign="center" valign="center" backgroundColor="yellow" />
	...
	def yellow(self):
		self.piconPositionToInstall = "dtv"
		self.choiceBoxPiconSize()
