...
class CommonChannelListScreen(Screen):
	skin = """
		<screen position="center,center" size="860,340" >
			...
			<widget name="newVersion3x" 	position="600,150" size="290,25" font="Regular;18" halign="left" />
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			self["status"].setText('Status: Usuwanie starych list')
...
class Djcrash(Screen):
	...
	def installChList(self, html):
		...
		if safe_zip_extract(r"/tmp/lista.zip", r"/tmp/chList/"):
			...
			shutil.rmtree('/tmp/chList', ignore_errors=True)
			try: os.remove('/tmp/lista.zip')
			except: pass
...
class CommonPiconListScreen(Screen):
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			try: os.makedirs(self.piconLocationToInstall + "/picon")
			except: pass
