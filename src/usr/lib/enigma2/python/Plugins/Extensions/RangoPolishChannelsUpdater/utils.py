	...
	with tarfile.open(tar_file) as tar:
		for member in tar.getmembers():
			if not is_path_safe(path, os.path.join(path, member.name)):
				raise Exception("Path traversal detected in tar: " + member.name)
		tar.extractall(path)
	...
	with zipfile.ZipFile(zip_file) as zf:
		for member in zf.namelist():
			if not is_path_safe(path, os.path.join(path, member)):
				raise Exception("Path traversal detected in zip: " + member)
		zf.extractall(path)
...
def verify_md5(filepath, expected_md5):
	if expected_md5 == "0" * 32 or expected_md5 == "":
		return True
	md5 = hashlib.md5()
...
class CommonChannelListScreen(Screen):
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			...
			if os.path.exists('/etc/enigma2/lamedb'): os.remove('/etc/enigma2/lamedb')
			if os.path.exists('/etc/enigma2/satellites.xml'): os.remove('/etc/enigma2/satellites.xml')
			if os.path.exists('/etc/enigma2/whitelist'): os.remove('/etc/enigma2/whitelist')
			if os.path.exists('/etc/enigma2/blacklist'): os.remove('/etc/enigma2/blacklist')
			if os.path.exists('/etc/enigma2/revision'): os.remove('/etc/enigma2/revision')
