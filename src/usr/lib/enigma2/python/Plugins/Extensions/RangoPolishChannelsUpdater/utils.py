...
def safe_tar_extract(tar_path, dest_path):
	if not os.path.exists(tar_path):
		return
	with tarfile.open(tar_path, "r:gz") as tar:
		for member in tar.getmembers():
			member_path = os.path.join(dest_path, member.name)
			if not is_path_safe(dest_path, member_path):
				raise Exception("Attempted Path Traversal in Tar File: %s" % member.name)
		# Enigma2 images often use older Python versions, numeric_owner might not be supported
		kwargs = {}
		if hasattr(tar, 'numeric_owner'):
			kwargs['numeric_owner'] = True
		tar.extractall(dest_path, **kwargs)
...
def safe_zip_extract(zip_path, dest_path):
	...
	with zipfile.ZipFile(zip_path, 'r') as zf:
		for member in zf.infolist():
			member_path = os.path.join(dest_path, member.filename)
			if not is_path_safe(dest_path, member_path):
				raise Exception("Attempted Path Traversal in Zip File: %s" % member.filename)
...
class CommonChannelListScreen(Screen):
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			...
			temp_extract = "/tmp/chList_extract/"
			if os.path.exists(temp_extract): shutil.rmtree(temp_extract)
			os.makedirs(temp_extract)

			safe_tar_extract(plik, temp_extract)
...
class Djcrash(Screen):
	...
	def downloadFromPKT(self):
		self["status"].setText("Status: Pobieranie listy " + self.chooseList)
		downloadPage(("http://addon.pkteam.pl/channel_lists_e2/enigma2_list_by_djcrash-" + self.newVersion + ".zip").encode('utf-8'), '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.errorUpdate)
	...
	def installChList(self, html):
		...
		if not os.path.exists(src_dir):
			self["status"].setText('Status: Błąd struktury archiwum !')
			return
			...
			with open('/proc/mounts', 'r') as f:
				for line in f.readlines():
					...
					if mount_point == '/media/cf':
						...
						mysd = '/media/card/'
					elif mount_point in ('/hdd', '/media/hdd'):
						myhdd = '/media/hdd/'
