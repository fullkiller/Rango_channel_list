...
def is_path_safe(base, target):
	...
	try:
		...
		# Fallback for older python or different drives
...
def safe_tar_extract(tar, path=".", members=None, **kwargs):
	for member in tar.getmembers():
		...
		if not is_path_safe(path, member_path):
			raise Exception("Unsafe path in tarball: %s" % member.name)
	...
	def red(self):
		...
		try:
			if self.url1x != '' and self.canDownload == 1:
				...
				self["status"].setText('Status: Czekaj na wersję...')
		except:
