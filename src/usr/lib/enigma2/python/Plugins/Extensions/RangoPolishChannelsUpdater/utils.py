...
def is_path_safe(base, target):
	base = os.path.normpath(os.path.realpath(base))
...
def safe_tar_extract(filepath, dest):
	with tarfile.open(filepath, "r:gz") as t:
		for member in t.getmembers():
			if is_path_safe(dest, os.path.join(dest, member.name)):
				t.extract(member, dest)

def safe_zip_extract(filepath, dest):
	with zipfile.ZipFile(filepath, "r") as z:
		for member in z.infolist():
			if is_path_safe(dest, os.path.join(dest, member.filename)):
...
class CommonChannelListScreen(Screen):
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			...
			to_remove = ['/etc/enigma2/*.tv', '/etc/enigma2/*.radio', '/etc/enigma2/lamedb', '/etc/enigma2/satellites.xml', '/etc/enigma2/whitelist', '/etc/enigma2/blacklist', '/etc/enigma2/revision']
...
class CommonPiconListScreen(Screen):
		...
		if os.path.exists('/proc/mounts'):
			with open('/proc/mounts', 'r') as f:
				for line in f:
					parts = line.split()
					if len(parts) >= 2:
						mounts[parts[1]] = parts[1]

		menu = []
		for mount in ['/media/cf', '/media/usb', '/media/usb2', '/media/usb3', '/media/card', '/media/hdd', '/media/hdd2']:
			if mount in mounts:
				menu.append((_("%s (%s/%s/picon)") % (mount, mount, self.piconTypeToInstall), mount + "/" + self.piconTypeToInstall))
				menu.append((_("%s (%s/picon)") % (mount, mount), mount))
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			replacements = ["ZZPiconGLASS", "ZZPiconBLACK", "ZZPiconRJ", "ZZPiconChristmas"]
			for r in replacements:
				if self.piconTypeToInstall == r:
					dest = dest.replace(r, "ZZPicon")

			if self.piconTypeToInstall in ("XPiconsBLACK", "SetoXPicons"):
				dest = dest.replace(self.piconTypeToInstall, "XPicons")
...
class ChannelListDownloadTask(Task):
	...
	def run(self, callback):
		...
		self.download = downloadWithProgress(self.url.encode(), self.path.encode())
