...
def is_path_safe(base, path):
	try:
		...
		if real_base == '/':
			allowed_prefixes = ['/usr', '/etc', '/var', '/lib', '/bin', '/sbin', '/share']
			return any(real_path.startswith(p) for p in allowed_prefixes)
...
def safe_tar_extract(tar_file, path=".", **kwargs):
	with tarfile.open(tar_file, "r:gz") as tar:
		for member in tar.getmembers():
			if not is_path_safe(path, member.name):
				print("[ChannelListUpdate] Forbidden path in tar: %s" % member.name)
				continue
...
def safe_zip_extract(zip_file, path="."):
	with zipfile.ZipFile(zip_file, 'r') as zf:
		for member in zf.infolist():
			if not is_path_safe(path, member.filename):
				print("[ChannelListUpdate] Forbidden path in zip: %s" % member.filename)
				continue
...
class Djcrash(Screen):
	...
	def downloadFromUpload(self):
		...
		# PKT server seems more reliable or at least it is the secondary option in many plugins
		# If this fails, we go to PKT, but the logic was recursive. Let's make PKT primary or fix fallback.
