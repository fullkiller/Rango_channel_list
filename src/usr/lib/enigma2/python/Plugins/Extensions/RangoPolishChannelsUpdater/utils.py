...
def is_path_safe(base, path):
	...
	if base == '/':
		...
		return any(path.startswith(os.path.realpath(a)) for a in allowed)
	return True
...
class CommonPiconListScreen(Screen):
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			dest_picon_dir = os.path.join(self.piconLocationToInstall, "picon")
			if not os.path.exists(dest_picon_dir):
				os.makedirs(dest_picon_dir)
