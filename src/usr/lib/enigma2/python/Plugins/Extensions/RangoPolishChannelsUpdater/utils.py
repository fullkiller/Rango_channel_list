...
class CommonChannelListScreen(Screen):
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			...
			files_to_remove = ['/etc/enigma2/*.tv', '/etc/enigma2/*.radio', '/etc/enigma2/lamedb', '/etc/enigma2/satellites.xml', '/etc/enigma2/whitelist', '/etc/enigma2/blacklist', '/etc/enigma2/revision']
...
class Djcrash(Screen):
	...
	def downloadFromUpload(self):
		...
		downloadPage(("http://upload.sat-elita.net.pl/download.php?id=" + self.chID).encode('utf-8'), '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.errorDownloadFallback)
	...
	def errorDownloadFallback(self, failure):
