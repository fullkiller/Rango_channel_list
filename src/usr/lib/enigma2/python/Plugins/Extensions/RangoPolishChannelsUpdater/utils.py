	...
	if realtarget == '/':
		...
		for prefix in allowed_prefixes:
			if realpath.startswith(prefix):
				return True
		return False
	return os.path.commonpath([realtarget, realpath]) == realtarget

def safe_tar_extract(tar, path=".", members=None, **kwargs):
	for member in tar.getmembers():
		member_path = os.path.join(path, member.name)
		if not is_path_safe(member_path, path):
			print("[ChannelListUpdate] Security Warning: Path traversal detected in tarball!")
			continue
		tar.extract(member, path, **kwargs)

def safe_zip_extract(zf, path=".", members=None):
	for member in zf.infolist():
		member_path = os.path.join(path, member.filename)
		if not is_path_safe(member_path, path):
			print("[ChannelListUpdate] Security Warning: Path traversal detected in zip!")
			continue
...
class CommonPiconListScreen(Screen):
	skin = """
		<screen position="center,center" size="1150,340" >
			...
			<widget name="key_red"		position="270,230" size="280,40" font="Regular;24" halign="center" valign="center" backgroundColor="red" />
			<widget name="key_green"	position="570,230" size="280,40" font="Regular;24" halign="center" valign="center" backgroundColor="green" />
			<widget name="status"		position="5,300" size="1150,40" font="Regular;24" halign="center" valign="center" backgroundColor="blue" />
		</screen>"""
