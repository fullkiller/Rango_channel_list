...
def safe_tar_extract(tar, path):
	for member in tar.getmembers():
		if not is_path_safe(os.path.join(path, member.name), path):
			raise Exception("Illegal tar archive member: %s" % member.name)
	tar.extractall(path)

def safe_zip_extract(zf, path):
	for member in zf.infolist():
		if not is_path_safe(os.path.join(path, member.filename), path):
			raise Exception("Illegal zip archive member: %s" % member.filename)
	zf.extractall(path)
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			with tarfile.open(name = plik, mode = "r:gz") as t:
				...
				dest_picon_path = os.path.join(self.piconLocationToInstall, "picon")
				if not os.path.exists(dest_picon_path):
					os.makedirs(dest_picon_path)
