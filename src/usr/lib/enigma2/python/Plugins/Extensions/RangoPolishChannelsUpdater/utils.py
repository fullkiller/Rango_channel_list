...
class CommonPiconListScreen(Screen):
	...
	def downloadCallBack(self, html, plik="/tmp/picon.tgz"):
		try:
			...
			for pattern in ["ZZPiconGLASS", "ZZPiconBLACK", "ZZPiconRJ", "ZZPiconChristmas"]:
				if self.piconTypeToInstall == pattern:
					dest = dest.replace(pattern, "ZZPicon")
			for pattern in ["XPiconsBLACK", "SetoXPicons"]:
				if self.piconTypeToInstall == pattern:
					dest = dest.replace(pattern, "XPicons")
...
class ChannelListDownloadTask(Task):
	...
	def run(self, callback):
		...
		self.download = downloadWithProgress(self.url.encode('utf-8'), self.path)
