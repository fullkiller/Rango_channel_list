...
class CommonChannelListScreen(Screen):
	...
	def red(self):
		self["status"].show()
		try:
			if hasattr(self, 'url1x') and self.url1x != '' and self.canDownload == 1:
	...
	def green(self):
		self["status"].show()
		try:
			if hasattr(self, 'url2x') and self.url2x != '' and self.canDownload == 1:
				self["status"].setText('Status: Pobieranie listy')
...
class Djcrash(Screen):
	...
	def getNewChListVersion(self, html):
		if not os.path.exists("/tmp/revision"):
			return
		...
		try:
			...
			if 'version' in new:
				...
				if ltype == "1x":
					...
					self.newVersion3 = new['version'].strip()
				self.canDownload = 1
		except Exception as error:
			print(error)
	 #	except:
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			if self.piconTypeToInstall in ("ZZPiconGLASS", "ZZPiconBLACK", "ZZPiconRJ", "ZZPiconChristmas"):
				self.piconLocationToInstall = self.piconLocationToInstall.replace("ZZPiconGLASS", "ZZPicon").replace("ZZPiconBLACK", "ZZPicon").replace("ZZPiconRJ", "ZZPicon").replace("ZZPiconChristmas", "ZZPicon")
			if self.piconTypeToInstall in ("XPiconsBLACK", "SetoXPicons"):
				self.piconLocationToInstall = self.piconLocationToInstall.replace("XPiconsBLACK","XPicons").replace("SetoXPicons","XPicons")
