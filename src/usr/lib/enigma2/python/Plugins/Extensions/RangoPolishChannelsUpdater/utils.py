	...
	with tarfile.open(tar_file, 'r:gz') as tar:
		for member in tar.getmembers():
			if not is_path_safe(path, os.path.join(path, member.name)):
				print("[ChannelListUpdate] Security alert: path traversal detected in tar archive!")
				continue
...
def safe_zip_extract(zip_file, path):
	with zipfile.ZipFile(zip_file, 'r') as zf:
		for member in zf.infolist():
			if not is_path_safe(path, os.path.join(path, member.filename)):
				print("[ChannelListUpdate] Security alert: path traversal detected in zip archive!")
				continue
