...
def is_path_safe(base, target, allowed_prefixes=None):
	real_base = os.path.realpath(base)
	real_target = os.path.realpath(target)
	if real_base == '/':
		if not allowed_prefixes:
			# If no prefixes provided and base is root, we allow common Enigma2 and media paths
			allowed_prefixes = ["/etc/enigma2/", "/usr/share/enigma2/", "/media/"]
		for prefix in allowed_prefixes:
			if real_target.startswith(os.path.realpath(prefix)):
				return True
		return False
...
def safe_tar_extract(tar_file, path, allowed_prefixes=None):
	with tarfile.open(tar_file, 'r:gz') as tar:
		for member in tar.getmembers():
			target_path = os.path.join(path, member.name)
			if is_path_safe(path, target_path, allowed_prefixes):
				tar.extract(member, path)
			else:
				print("[ChannelListUpdate] skipping unsafe path in tar: %s" % member.name)

def safe_zip_extract(zip_file, path, allowed_prefixes=None):
	with zipfile.ZipFile(zip_file, 'r') as zf:
		for member in zf.infolist():
			...
			if is_path_safe(path, target_path, allowed_prefixes):
				...
				print("[ChannelListUpdate] skipping unsafe path in zip: %s" % member.filename)
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			...
			for f in ['lamedb', 'satellites.xml', 'whitelist', 'blacklist', 'revision']:
				path = os.path.join('/etc/enigma2/', f)
				if os.path.exists(path):
					os.remove(path)
			self["status"].setText('Status: Rozpakowywanie')
			safe_tar_extract(plik, "/", ["/etc/enigma2/", "/usr/share/enigma2/"])
			reloadChannelList()
...
class Djcrash(Screen):
	...
	def downloadFromPKT(self):
		...
		downloadPage("http://addon.pkteam.pl/channel_lists_e2/enigma2_list_by_djcrash-" + self.newVersion + ".zip", '/tmp/lista.zip').addCallback(self.installChList)
