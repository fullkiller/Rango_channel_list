...
def safe_tar_extract(archive, path, **kwargs):
	if not os.path.exists(path):
		os.makedirs(path)
	with tarfile.open(archive, 'r:gz') as t:
		for member in t.getmembers():
			target_path = os.path.join(path, member.name)
			if not is_path_safe(target_path, path):
				continue
			t.extract(member, path, **kwargs)

def safe_zip_extract(archive, path):
	if not os.path.exists(path):
		os.makedirs(path)
...
class Djcrash(Screen):
	...
	def getNewChListVersion(self, html):
		content = open("/tmp/revision", 'r').read()
		paths = content.split("\n")
		for path in paths:
