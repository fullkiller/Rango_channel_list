	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			...
			for member in t.getmembers():
				if not os.path.normpath(os.path.join("/", member.name)).startswith("/"):
					raise Exception("Unsafe tar member: %s" % member.name)
			t.extractall("/")
			t.close()
			os.sync()
			reloadChannelList()
	...
	def installChList(self, html):
		...
		try:
			...
			for member in zf.infolist():
				if not os.path.normpath(os.path.join("/tmp/chList/", member.filename)).startswith("/tmp/chList/"):
					raise Exception("Unsafe zip member: %s" % member.filename)
...
class CommonPiconListScreen(Screen):
	...
	def prepareDownload(self):
		...
		try:
			...
			self["status"].setText('Status: Nie mogę się połączyć z serwerem !')
			if self.baseUrl and self.url1 and self.url2 and self.url3:
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			if not os.path.exists(os.path.join(self.piconLocationToInstall, "picon")):
				os.makedirs(os.path.join(self.piconLocationToInstall, "picon"))

			self["status"].setText('Status: Instalowanie')
			for member in t.getmembers():
				if not os.path.normpath(os.path.join(self.piconLocationToInstall, member.name)).startswith(os.path.abspath(self.piconLocationToInstall)):
					raise Exception("Unsafe tar member: %s" % member.name)
...
class ChannelListDownloadTask(Task):
	...
	def run(self, callback):
		...
		self.download = downloadWithProgress(self.url.encode(), self.path.encode() if isinstance(self.path, str) else self.path)
