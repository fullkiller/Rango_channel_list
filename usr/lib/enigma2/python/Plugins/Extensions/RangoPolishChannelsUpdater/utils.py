...
class Djcrash(Screen):
	...
	def downloadFromUpload(self, failure_instance=None):
		self["status"].setText("Status: Pobieranie listy " + self.chooseList)
		downloadPage(("http://upload.sat-elita.net.pl/download.php?id=" + self.chID).encode('utf-8'), b'/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFailed)

	def downloadFromPKT(self, failure_instance=None):
		self["status"].setText("Status: Alternatywne pobieranie listy " + self.chooseList)
		downloadPage(("http://addon.pkteam.pl/channel_lists_e2/enigma2_list_by_djcrash-" + self.newVersion + ".zip").encode('utf-8'), b'/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromUpload)

	def downloadFailed(self, failure_instance=None):
...
class CommonPiconListScreen(Screen):
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			self["status"].setText('Status: Instalowanie')
			t.extractall(self.piconLocationToInstall)
			os.system('sync')
