	...
	def installChList(self, html):
		...
		try:
			...
			for member in zf.infolist():
				if not (os.path.abspath(os.path.join(r"/tmp/chList/", member.filename)).startswith(os.path.abspath(r"/tmp/chList/"))):
...
class CommonPiconListScreen(Screen):
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			t = tarfile.open(name = plik, mode = "r:gz")
			for member in t.getmembers():
				if not (os.path.abspath(os.path.join(self.piconLocationToInstall, member.name)).startswith(os.path.abspath(self.piconLocationToInstall))):
					raise Exception("Attempted Path Traversal in Tar File")
...
class ChannelListDownloadTask(Task):
	...
	def run(self, callback):
		...
		self.download = downloadWithProgress(self.url.encode('utf-8'), self.path.encode('utf-8') if isinstance(self.path, str) else self.path)
