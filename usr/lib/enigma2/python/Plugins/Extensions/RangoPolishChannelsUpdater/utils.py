...
import traceback
import re, os, tarfile, zipfile, shutil, glob, hashlib
	...
	try:
		...
		if base != '/':
			...
			for a in allowed:
				...
				if path.startswith(aa + os.sep) or path == aa:
					return True
	except Exception:
		pass
	return False
...
def safe_tar_extract(filepath, extract_path):
	try:
		if not os.path.exists(filepath):
			return False
		...
		print("Error extracting tar: %s" % str(e))
		return False

def safe_zip_extract(filepath, extract_path):
	try:
		if not os.path.exists(filepath):
			return False
		...
		print("Error extracting zip: %s" % str(e))
		return False
...
class CommonPiconListScreen(Screen):
			...
			with open('/proc/mounts', 'r') as f:
				for line in f:
					if '/media/cf' in line:
						...
						myhdd2 = '/media/hdd2/'
					elif '/media/hdd' in line or '/hdd' in line:
						myhdd = '/media/hdd/'
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			dest_picon = os.path.join(self.piconLocationToInstall, "picon")
