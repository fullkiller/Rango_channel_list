...
class CommonChannelListScreen(Screen):
	...
	def versionInfoCallBack(self, html, ltype="1x"):
		try:
			new = dict([ l.split(':',1) for l in str(html.decode('utf-8')).split("\n") if l.find(":") > 0])
			if 'version' in new:
...
class Djcrash(Screen):
	...
	def downloadFromUpload(self, failure=None):
		...
		downloadPage(("http://upload.sat-elita.net.pl/download.php?id=" + self.chID).encode('utf-8'), b'/tmp/lista.zip').addCallback(self.installChList).addErrback(self.errorUpdate)
