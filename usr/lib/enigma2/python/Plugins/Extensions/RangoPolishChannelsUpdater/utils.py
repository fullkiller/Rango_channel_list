...
class Djcrash(Screen):
	...
	def downloadFromUpload(self, failure=None):
		...
		downloadPage(url, b'/tmp/lista.zip').addCallback(self.installChList).addErrback(self.errorUpdate)
	...
	def downloadFromPKT(self, failure=None):
		...
		if isinstance(url, str):
			url = url.encode('utf-8')
		downloadPage(url, b'/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromUpload)
