...
class ChannelListUpdateMenu(Screen):
	...
	def check_updates(self, tryb = 1):
		try:
			...
			if tryb == 0:
				getPage(url).addCallback(self.versionInfoCallBack, True).addErrback(self.errorUpdate)
			else:
				getPage(url).addCallback(self.versionInfoCallBack, False).addErrback(self.errorUpdate)
	...
	def versionInfoCallBack(self, html, quiet=False):
		...
		try:
			...
			if config.has_section('plugin'):
				self.newVersion = config.get('plugin', 'Version', fallback=version)
				self.newFile = config.get('plugin', 'File', fallback='')
				self.fileUrl = config.get('plugin', 'Url', fallback='')
				self.MD5 = config.get('plugin', 'MD5', fallback='')
		except Exception as e:
			print("[Rango] ConfigParser failed, falling back to manual split: %s" % str(e))
			paths = content.split("\n")
			for path in paths:
				if " = " in path:
...
class HSWG(CommonChannelListScreen):
	def __init__(self, session, args = None):
		...
		self.url1 = 'https://raw.githubusercontent.com/fullkiller/Rango_channel_list/main/latest4x.txt'
		self.url2 = 'https://raw.githubusercontent.com/fullkiller/Rango_channel_list/main/latest8x.txt'
...
class K80(CommonChannelListScreen):
	def __init__(self, session, args = None):
		...
		self.url1 = 'https://raw.githubusercontent.com/fullkiller/Rango_channel_list/main/latest6x.txt'
...
class Fk(CommonChannelListScreen):
	def __init__(self, session, args = None):
		...
		self.url1 = 'https://raw.githubusercontent.com/fullkiller/Rango_channel_list/main/latest2x.txt'
...
class Master(CommonChannelListScreen):
	def __init__(self, session, args = None):
		...
		self.url1 = 'https://raw.githubusercontent.com/fullkiller/Rango_channel_list/main/stalker1.txt'
...
class ChannelListUpdateUpdater(Screen):
	...
	def startUpdate(self):
		...
		if isinstance(dlfile, str):
			dlfile = dlfile.encode('utf-8')
		self.download = downloadWithProgress(url, dlfile)
	...
	def downloadFinished(self, string = ''):
		...
		os.system('cd /tmp/; tar -xzvf update_rango.tar.gz -C /; sync')
		os.system('rm -rf /tmp/update_rango*.*')
		os.system('sync')
