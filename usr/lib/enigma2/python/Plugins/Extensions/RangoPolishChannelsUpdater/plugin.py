...
from utils import reloadChannelList, CommonChannelListScreen, Djcrash, CommonPiconListScreen, safe_tar_extract, verify_md5
import os
...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		if self.md5 and self.md5 != "00000000000000000000000000000000":
			if not verify_md5('/tmp/update_rango.tar.gz', self.md5):
				self['status'].setText(_('Błąd weryfikacji MD5!'))
				return
		self['status'].setText(_('Instalowanie aktualizacji!'))
		if safe_tar_extract('/tmp/update_rango.tar.gz', '/'):
			os.system('sync')
