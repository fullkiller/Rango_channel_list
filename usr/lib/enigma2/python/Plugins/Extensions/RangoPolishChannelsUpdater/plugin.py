...
class ChannelListUpdateMenu(Screen):
	...
	def versionInfoCallBack(self, html, tryb):
		...
		try:
			...
			self.newVersion = cp.get("plugin", "Version")
			self.newFile = cp.get("plugin", "File")
			self.fileUrl = cp.get("plugin", "Url")
			self.MD5 = cp.get("plugin", "MD5")
		except:
			paths = content.split("\n")
			for path in paths:
...
class ChannelListUpdateUpdater(Screen):
	...
	def downloadFinished(self, string = ''):
		...
		from .utils import safe_tar_extract, sync_filesystem
		import tarfile
		try:
