...
import traceback
import re, os, tarfile, zipfile, shutil
...
def safe_tar_extract(tar_file, path):
	with tarfile.open(tar_file, 'r:gz') as tar:
		for member in tar.getmembers():
			if is_path_safe(path, os.path.join(path, member.name)):
				tar.extract(member, path)

def safe_zip_extract(zip_file, path):
	with zipfile.ZipFile(zip_file, 'r') as zip_ref:
		for member in zip_ref.namelist():
			if is_path_safe(path, os.path.join(path, member)):
...
class CommonChannelListScreen(Screen):
	...
	def versionInfoCallBack(self, html, ltype="1x"):
		try:
			...
			new = dict([ l.strip().split(':',1) for l in html.split("\n") if l.find(":") > 0])
			if 'version' in new:
				self["newVersion%s" % ltype].setText(new['version'].strip())
				if ltype == "1x":
					self.url1x = new['url']
	...
	def downloadCallBack(self, html, plik = "/tmp/chList1x1.tgz"):
		try:
			### should be replaced perhaps by shutils os.system is eating memory....
			if self.cleanup_before_install:
				# os.system('rm /etc/enigma2/*.tv')
				os.system('rm /etc/enigma2/*.radio')
				os.system('rm /etc/enigma2/lamedb')
				os.system('rm /etc/enigma2/satellites.xml')
				os.system('rm /etc/enigma2/whitelist')
				os.system('rm /etc/enigma2/blacklist')
				os.system('rm /etc/enigma2/revision')
...
class Djcrash(Screen):
	...
	def downloadFromUpload(self):
		...
		downloadPage("http://upload.sat-elita.net.pl/download.php?id=" + self.chID, '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromPKT_once)

	def downloadFromPKT(self):
		self["status"].setText("Status: Alternatywne pobieranie listy " + self.chooseList)
		downloadPage("http://addon.pkteam.pl/channel_lists_e2/enigma2_list_by_djcrash-" + self.newVersion + ".zip", '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.downloadFromUpload_once)

	def downloadFromPKT_once(self, failure=None):
		self["status"].setText("Status: Alternatywne pobieranie listy " + self.chooseList)
		downloadPage("http://addon.pkteam.pl/channel_lists_e2/enigma2_list_by_djcrash-" + self.newVersion + ".zip", '/tmp/lista.zip').addCallback(self.installChList).addErrback(self.errorUpdate)
	...
	def installChList(self, html):
		...
		if self.cleanup_before_install:
			...
			os.system('rm /etc/enigma2/blacklist')
			os.system('rm /etc/enigma2/revision')
		os.system('cp -a /tmp/chList/list_by_djcrash-e2_' + self.chooseList + '/* /etc/enigma2/')
		os.system('cp -a /tmp/chList/revision /etc/enigma2/')
		## End of dirty code....
		reloadChannelList()
		os.system('rm -rf /tmp/chList')
	...
	def downloadCallBack(self, html, plik = "/tmp/picon.tgz"):
		try:
			...
			if self.piconTypeToInstall in ("ZZPiconGLASS"):
				self.piconLocationToInstall = self.piconLocationToInstall.replace("ZZPiconGLASS", "ZZPicon").replace("ZZPiconGLASS", "ZZPicon")
			if self.piconTypeToInstall in ("ZZPiconBLACK"):
				self.piconLocationToInstall = self.piconLocationToInstall.replace("ZZPiconBLACK", "ZZPicon").replace("ZZPiconBLACK", "ZZPicon")
			if self.piconTypeToInstall in ("ZZPiconRJ"):
				self.piconLocationToInstall = self.piconLocationToInstall.replace("ZZPiconRJ", "ZZPicon").replace("ZZPiconRJ", "ZZPicon")
			if self.piconTypeToInstall in ("ZZPiconChristmas"):
				self.piconLocationToInstall = self.piconLocationToInstall.replace("ZZPiconChristmas", "ZZPicon").replace("ZZPiconChristmas", "ZZPicon")
			...
			os.system("mkdir -p " + self.piconLocationToInstall + "/picon")
			self["status"].setText('Status: Instalowanie')
